...
import airflow.hara_bin.cwl_tools.hara_engine.hara_workflow_job

from airflow.hara_bin.cwl_tools.hara_engine.hara_workflow_job import HaraWorkflowJob
from airflow.hara_bin.cwl_tools.hara_engine import hara_load_tool
...
def default_make_tool(toolpath_object: CommentedMap, loadingContext: LoadingContext) -> Process:
    ...
    if "class" in toolpath_object:
        ...
        if toolpath_object["class"] == "Workflow":
            ...
            return HaraWorkflow(toolpath_object, loadingContext)
...
class HaraWorkflow(Process):
    ...
    def job(
        ...
        job = HaraWorkflowJob(self, runtimeContext)
        yield job
    ...
    def __init__(
        ...
        try:
            if isinstance(toolpath_object["run"], CommentedMap):
                ...
                self.embedded_tool = hara_load_tool.load_tool(toolpath_object["run"], loadingContext)
        except ValidationException as vexc:
