...
from sqlalchemy import BigInteger, Column, Index, LargeBinary, String, and_, or_, SmallInteger
...
class HaraSerializedDagModel(Base):
    ...
    * ``[core] compress_serialized_dags``:
    ...
    __tablename__ = "hara_serialized_dag"
    ...
    dag_owner = Column(String(250), nullable=True)
    cwl_json = Column("cwl_json", sqlalchemy_jsonfield.JSONField(json=json), nullable=True)
    last_updated = Column(UtcDateTime, nullable=False)
    create_time = Column(UtcDateTime, nullable=False)
    row_status = Column(SmallInteger, nullable=False)  # in db it's not nullable, populate it with trigger.
    ...
    dag_runs = relationship(
        ...
        backref=backref("hara_serialized_dag", uselist=False, innerjoin=True),
    )
    ...
    def __init__(self, dag: DAG,
        ...
        # hara change starts: add cwl_json and dag_owner to hara_serialized_dag
        if cwl_json is not None:
            self.cwl_json = json.dumps(cwl_json, sort_keys=True).encode("utf-8")
    ...
    def read_all_dags(cls, session: Session = None) -> dict[str, SerializedDAG]:
        ...
        serialized_dags = session.query(cls).filter(cls.row_status == 0).all()
    ...
    @classmethod
    @provide_session
    def get(cls, dag_id: str, session: Session = None) -> HaraSerializedDagModel | None:
        """
