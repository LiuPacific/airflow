...
# TODO: to config
cwl_root_dir = '/home/typingliu/airflow_v21/hara_cwl_buffer'


class CwlCommandLineToolOperator(BaseOperator):
    r"""
    ...
    # template_fields = ('bash_command', 'env') # TODO tp: what does it work for?
    ...
    def __init__(
        ...
        **kwargs,  # TODO tpL what does it work for?
    ) -> None:
    ...
    def get_work_dir(self, context):
        global cwl_root_dir
        dag_run = context.get('dag_run')
        run_id = dag_run.run_id
        self.log.warning('harada--------------get_work_dir')
        self.log.warning('harada: dag run id is %s', run_id)
        path_safe_run_id = run_id.replace(':', '_').replace(' ', '_').replace('+', '_')
        self.log.warning('harada: path_safe_run_id is %s', path_safe_run_id)
        self.log.warning('harada--------------get_work_dir')
        work_dir_path = os.path.join(cwl_root_dir, path_safe_run_id)
    ...
    def execute(self, context):
        ...
        self.log.warning('tp--------------')
        # get workdir:
        work_dir_path = self.get_work_dir(context)

        # cwltool echo.cwl.yaml --message_text="hello typing"
        # cwltool echo.cwl.yaml job_path.yaml
        # cwl_commands = ['cwltool', self.cwl_path, self.job_path]
        # cwl_command = f'cwltool --outdir={work_dir_path} {self.cwl_path} {self.job_path}'
        # cwl_command = f'cwltool --basedir={work_dir_path} --outdir={work_dir_path} {self.cwl_path} {self.job_path}'
        test_path = '/home/typingliu/workspace/tpy/airflow21/airflow/airflow/hara_bin/hara_dags/inputfile_localcwl_local_dag/a'
        cwl_command = f'cwltool --outdir={work_dir_path} {self.cwl_path} {self.job_path}'
        result = self.subprocess_hook.run_command(
            command=['bash', '-c', cwl_command],
            env=env,
